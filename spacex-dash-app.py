...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        # Filter für spezifischen Startplatz
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        # Zähle Erfolge (1) und Misserfolge (0)
        df_site = filtered_df.groupby(['class']).size().reset_index(name='counts')
        fig = px.pie(df_site, values='counts', 
                     names='class', 
...
def get_scatter_chart(entered_site, payload_range):
    ...
    # Filtere nach Payload-Bereich
    mask = (spacex_df['Payload Mass (kg)'] >= low) & (spacex_df['Payload Mass (kg)'] <= high)
    df_scatter = spacex_df[mask]
    
    if entered_site == 'ALL':
        fig = px.scatter(df_scatter, x="Payload Mass (kg)", y="class", 
                         color="Booster Version Category",
                         title='Correlation between Payload and Success for all Sites')
        return fig
    else:
        # Filtere nach spezifischem Startplatz
        df_scatter = df_scatter[df_scatter['Launch Site'] == entered_site]
        fig = px.scatter(df_scatter, x="Payload Mass (kg)", y="class", 
                         color="Booster Version Category",
